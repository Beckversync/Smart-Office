...
constexpr char CURRENT_FIRMWARE_TITLE[] = "Energy_meter";
...
void readEnergyMeterTask(void *pvParameters) {
  int amperage = 15;
  int frequency = 60;
  float energy = 130.3;
  float power = 0.0;
  float voltage = 215.0f;
  bool voltageIncrease = false;
  while (true) {
    //Real voltage reading would go here and depends on your hardware setup.

    // Simple Simulate voltage changes 
    if (voltageIncrease) {
      voltage += 0.1f;
      if (voltage >= 230.0f) {
        voltageIncrease = false;
      }
    } else {
      voltage -= 0.1f;
      if (voltage <= 215.0f) {
        voltageIncrease = true;
      }
    }
    //Randomly change amperage
    amperage = random(15, 25); // Simulate amperage between 15 and 25 A
    //Randomly change frequency
    frequency = random(58, 62); // Simulate frequency between 58 and 62 Hz
    // Calculate power in watts
    power = voltage * amperage; // Calculate power in watts
    // Calculate energy in kWh (assuming this is a cumulative reading)
    energy += (power / 3600000.0f) * (30.0f / 60.0f); // Assuming 30 seconds interval, convert to kWh

    Serial.printf("Amperage: %d A, Frequency: %d Hz, Energy: %.2f kWh, Power: %.2f W, Voltage: %.2f V\n", amperage, frequency, energy, power, voltage);

    tb.sendTelemetryData("amperage", amperage);
    tb.sendTelemetryData("frequency", frequency);
    tb.sendTelemetryData("energy", energy);
...
void setup() {
  ...
  xTaskCreate(ThingsBoardTask, "TBTask", 8192, NULL, 1, NULL);
  //xTaskCreate(readEnergyMeterTask, "readEnergyMeterTask", 8192, NULL, 1, NULL);
}

void loop() {
  Serial.println("ESP32 Energy Meter Example");
  delay(2000);
}

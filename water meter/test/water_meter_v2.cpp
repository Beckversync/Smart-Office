...
constexpr char CURRENT_FIRMWARE_TITLE[] = "Water_meter";
...
constexpr char THINGSBOARD_SERVER[] = "Your_Local_ThingsBoard_Server"; //app.coreiot.io

constexpr char VOLTAGE_KEY[] = "voltage";
constexpr char WATER_KEY[] = "water";
...
void VotageTask(void *pvParameters) {
...
void WaterTask(void *pvParameters) {
  float water = 0.0f;
  while (true) {
    //Real water usage reading would go here and depends on your hardware setup.

    // Simple Simulate water usage changes
    tb.sendTelemetryData( WATER_KEY, water);
    Serial.printf("Water: %.2f L\n", water);
    water += 0.1f;
    vTaskDelay(pdMS_TO_TICKS(TELEMETRY_SEND_INTERVAL));
  }
}
...
void setup() {
  ...
  xTaskCreate(VotageTask, "VotageTask", 8192, NULL, 1, NULL);
  xTaskCreate(WaterTask, "WaterTask", 8192, NULL, 1, NULL);
}
